...
## Умные указатели
Можно чуть усложнить пример выше, и в качестве типа данных вектора использовать произвольный указатель.
...
При такой схеме возникает сразу несколько потенциальных проблем. Во-первых, если переменные *a* и *b* выйдут за пределы области видимости, их указатели сразу станут дефективными. Во-вторых, если заменить адреса на полученные в "куче", их придется затем освобождать явно.

    auto create() {
        vector<var*> v;
        v.push_back(new var{42});
        v.push_back(new var{69});
        return v;
    }
...
В-третьих, в случае каких-либо исключительных ситуаций придется отдельно бороться с утечками памяти.
...
Как видно из примера, код, написанный в таком стиле, требует слишком много дополнительных проверок и обработок, чтобы исключить возможность ошибок в работе программы. Этого можно избежать, применив идиому RAII.

    class SmartObj {
        vector<var*> v;
    public:
        // инкапсуляция освобождения ресурсов:
        ~SmartObj() {
            for (auto p : v) {
                delete p;
                p = nullptr;
        ...
        // инкапсуляция выделения ресурсов:
        void insert(int x) {
            v.push_back(new var{x});
        }
        // копирование необходимо запретить:
        SmartObj(const SmartObj&) = delete;
        SmartObj& operator=(const SmartObj&) = delete;
    };
В примере выше применяется правило "трех" для достижения нужного эффекта: теперь при выходе переменной типа SmartObj за пределы области видимости (независимо от причины, будь то возвращение из функции или исключение), все ресурсы освободятся автоматически.
    ...
    auto val = create();
    // явное очищение ресурсов больше не нужно
В языке С++ вместе со стандартом 2011 года в библиотеку memory было добавлено несколько классов, которые вносят в общую категорию "умных указателей". Два самых часто используемых класса -- это unique_ptr и shared_ptr.

Суть "умных" указателей сводится к инкапсуляции освобождения ресурсов, которые запрашиваются программой, аналогично примеру с RAII выше. Например, класс SmartObj можно заменить на стандартный shared_ptr:

    #include <shared_ptr>
    using varp = std::shared_ptr<var>;

    auto create() {
        varp v;
        v.push_back(std::make_shared<var>{42});
        v.push_back(std::make_shared<var>{69});
        return v;
    }
...
Указатель на объект var хранится в объекте типа std::shared_ptr, код "умного" объекта следит за всеми ссылками на указатель с помощью обычного счетчика, деструктор "умного" объекта проверяет счетчик и вызывает delete только в том случае, если счетчик равен "0" -- т.е. в коде программы ресурс больше нигде не нужен.

Класс std::unique_ptr также автоматически освободит ресурс, которым управляет, но в отличие от std::shared_ptr не позволяет копировать свой указатель на var, как было сделано в примере с классом SmartObj выше.
## Правило "нуля"
...
## Семантика переноса
Перенос был введен в язык для того, чтобы позволить программисту контролировать передачу ресурсов одного объекта другому и явно обозначать ситуации, где это происходит. Например, если какой-то контейнер выходит за пределы области видимости, и его ресурсы вот-вот будут очищены, их можно перенести в свободную переменную такого же типа, когда есть необходимость ресурсы сохранить. Это позволит избежать лишнего выделения памяти, заимствовав память уничтожаемого объекта.
...
В примере выше можно заметить, что код операторов присваивания для обоих случаев выглядит аналогично, за исплючением типа аргумента. 
...
Это замечание можно использовать для сокращения кода.
...
В этой версии класса всего одна перегрузка оператора присваивания:

    vector& operator=(vector other) {
        other.swap(*this);
        return *this;
    }
Здесь видно два отличия от кода для "правила 'пяти'": параметр *other* не является ссылкой, а передается по значению; в теле функции нет временного объекта *tmp*. Временный объект уже не нужен, потому что его роль играет параметр *other*. Инициализация параметра по значению создаст локальную переменную *other*, которая и будет временным объектом. Дальше, как и в ранних примерах, текущий объект *this* меняется ресурсами с временным *other*. Так, ответственность за правильную инициализацию параметра ложится на вызывающую сторону, которая делегирует конструирование объекта *other* соответствующему конструктору. Теперь, если вызывать оператор присваивания с аргументом из категории lvalue, то вызовется конструктор копирования, а если -- rvalue, вызовется конструктор переноса.
