...
В известной работе "Абстракции данных и иерархии" Барбара Лисков (в чью честь принцип назван) пишет,
...
Во многих языках программирования модули, в которые группируются классы и функции, представляются отдельным файлом. Включение этих файлов в ту или иную программу происходит по-разному в зависимости от языка программирования. Если в языке С (и традиционно - в  С++) директива *include* включает текст файла единым блоком, чтобы затем его компилировать как часть исходного кода всей программы, в других языках код модуля интерпретируется "на ходу" по мере необходимости. 

Но в обоих случаях могут возникнуть трудности, если в цепочку включений завивимостей закрадется цикл. Например, если в программе используется четыре модуля, которые зависят один от другого: GUI -> Comm -> Protocol -> Comm Error -- никаких проблем не возникнет, т.к. нет двух модулей, котрые бы зависели друг от друга.

Предположив, что разработчик модуля Comm Error не знает о том, что тот модуль является зависимостью каких-то других модулей, можно представить ситуацию, в которой он включит модуль GUI как зависимость для своего модуля, чтобы визуализировать работу с ошибками. Тогда, включая в программу модуль Comm Error, пользватели будут получать цикл, в котором зависимость этого модуля от GUI не может быть разрешена, так как GUI зависит от него самого. Более того, все зависимости модуля GUI теперь тоже будут включены в модуль Comm Error, хотя их функционал там бесполезен.

Выделяется два способа решения этой проблемы. Первый способ заключается в создании нового модуля, от которого будут зависеть два модуля, образующие цикл. В примере выше из модуля GUI необходимо вынести функционал, от которого зависит модуль CommError. Этот новый модуль будет зависимостью и для GUI, и для Comm Error, т.о. цикл, образованный ими исчезает. 

Конечно, проблема остается неразрешенной, если вынесенный функционал сам зависел от других модулей в цепочке GUI-Comm Error, т.е. данное решение не во всех сценариях применимо. Второй способ более универсальный, но требует дополнительных усилий: необходимо применить принцип инверсии зависимостей, создав соответствующий интерфейс. 

Вместо того, чтобы выносить сам фунционал зависимости, создается интерфейс, описывающий методы, которые нужны зависимому модулю. Модуль теперь зависит от абстракции, а не от конкретной реализации. При этом реализация зависит от интерфейса, и это разрывает цикл. Стоит отметить, что интерфейс определяется в том модуле, который пользуется его реализацией, а не в том модуле, где определена реализация, потому что реализация может меняться, и это не должно затрагивать сам интерфейс.
## Stable Dependencies
> Depend in the direction of stability

Что имеется ввиду под стабильностью? Стабильность заключается в том, сколько усилий требуется затратить на внесение изменений в модуль. Если в модуль постоянно вносятся изменения, которые ни с кем не надо согласовывать, то такой модуль по определению нестабилен. Если же для сколь угодно маленьких изменений нужно одобрение других членов команды или ответственных за модуль, модуль очень стабилен. Т.е. стабильность определяется тем, стоит ли ожидать, что зависимость будет изменена и вызовет каскадные изменения в зависимых модулях.

Один такой критерий стабильности -- количество текущих модулей, которые зависят от конкретного модуля. Если от модуля зависит много другого функционала, его изменение может вызвать проблемы: сломать другие модули, вызвать необходимость обновлять другие модули -- такой модуль со временем становится все стабильней (как говорит Мартин, "модуль имеет множество причин не изменяться"). Т.е. модуль отвечает перед зависящими от него модулями, сам при этом может быть независимым. 

Т.о. условную стабильность отдельно взятого модуля можно измерить, она определяется отношением исходящих зависимостей к входящим. Например, если у модуля нет входящих зависимостей, то он не отвечает ни перед чем, а следовательно -- он нестабилен (может изменяться когда угодно).

Мартин делает вывод, что если зависеть следует от стабильных модулей, то зависеть нужно от модулей, которые уже имеют можество зависимых модулей, потому что они вряд ли изменятся. Конечно, этот совет не является гарантией стабильности зависимости, т.к. разработчики могут в любой момент принять решение о внесении изменений в свой "стабильный" модуль (например, для исправления уязвимостей или недавно обнаруженных "багов"). Мартин и здесь позволяет себе исключение из правила: если критерий хорошего программного обеспечения -- легкость внесения изменений, это входит в прямое противоречие с этим принципом.
## Stable Abstractions
> Stable packages should be abstract packages

Противоречие, обнаруженное в предыдущем разделе, Мартин предлагает снять достаточно просто: необходимо все модули делать стабильными засчет интерфейсов, т.е. абстракций. Как уже постулировалось в DIP, абстракции создаются однажды и не меняются потом, т.к. являются простыми заявлениями требований и ограничений при использовании своих реализаций, отсюда следует то, что они безопасны для входящих зависимостей, что делает их сверх-стабильными. Любые изменения можно вносить засчет последующего расширения (см. OCP).

Т.о. элегантное решение Мартина заключается в том, что система проектируется из легко расширяемых стабильных абстракций и легко изменяемых нестабильных реализаций. Исследуя все перечисленные в этой главе принципы, можно было заметить тренд: все советы так или иначе сводились к созданию интерфейсов.

Уровень "абстрактности" модуля также можно измерить, он определяется отношением абстрактных классов к конкретным. Если модуль не содержит абстрактных классов, он полностью конкретный, а следовательно - априори нестабильный, и наоборот. Пользуясь этим критерием все модули условно разделяются на три группы: проблемные, сбалансированные и бесполезные.

Мартин называет это "абстракцией как функцией нестабильности": чем ниже уровень абстрактности, тем выше уровень нестабильности. Стабильный, но конкретный модуль потенциально вызовет много проблем, так как зависимость от такого модуля может вызвать необходимость часто изменять зависящие от него части системы. Нестабильный абстрактный модуль также мало применим на практике, так как противоречит своему прямому назначению как интерфейс: таким интерфейсом никто не пользуется, но при этом в него часто вносятся изменения -- это пустая трата времени. Таким образом, функция баланса как прямая лежит между точками абсолютной стабильной абстракции и абсолютной нестабильной реализации.

В завершение следует сказать, что Мартин вынашивал эти идеи не один год, а окончательно сформулировал их в самый интересный период развития идей ООП (см. https://okmij.org/ftp/Computation/Subtyping/). Поэтому для человека, знакомого с ООП в основном на основании работ Алана Кея принципы, описанные выше, могут показаться высосанными из пальца, но надо понимать, что они являются квинтессенцией того, как ООП понимали в программисты в своей массе тогда. Это становится особенно очевидным при ознакомлениями с шаблонами проектирования программного обеспечения "банды четырех", которые до сих пор считаются основой дизайна программного обеспечения. Рассмотрение этого пласта истории ООП выходит за рамки данного курса, но принципы SOLID являются хорошей отправной точкой для самостоятельного их изучения.