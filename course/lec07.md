...
Из этого есть интересное следствие. Любой тип или символ, который отвечает этим требованиям, может быть использован для определения *X*, как в примере ниже.
...
Шаблоны образуют формальную грамматику -- набор правил для замены символов или комбинаций символов, определяемых другими правилами, которые, в свою очередь, также определены какими-то правилами. Еще шаблоны позволяют писать определения, которые не зависят от конкретных типов, а зависят от абстракций. Аналогично типам, которые зависят от абстрактных типов в иерархиях наследования, шаблоны позволяют создавать семейства типов. Эти семейства типов можно использовать для написания полиморфного кода.
## Макросы и шаблоны
...
Аналогичный шаблон определяется так.
...
Необходимо небольшое отступление от конкретики шаблонов для того, чтобы коротко обсудить роль и виды абстракции в программировании. В рамках этого курса рассматривается две группы абстракций:
1. Функции (в контексте ООП -- "сообщения".)
2. Типы данных (в контексте ООП -- "классы".)

Абстракции используются для управления сложностью программ. Намного проще оперировать именами функций, которые определяют хорошо сформулированные интерфейся, чем блоками императивного кода. (При этом не стоит забывать, что суть информатики как раз в формализации понятия "процесс": императивного подхода -- знания о том, "как" что-то делается, в отличие от декларативного знания о том, "что является истиной".) Еще проще оперировать абстрактными интерфейсами без привязки к конкретным блокам кода, потому что такие интерфейсы легче комбинировать друг с другом.

Абстракции важны еще по одной причине. В информатике, в отличие от естественных наук, разработка новых решений происходит в отрыве от физических законов, управляющих окружающей реальностью. Так происходит потому, что информатика вырабатывает идеализированные компоненты -- компоненты, ограничением для которых выступают не физические условия, а степень ограниченности человеческого разума. Поэтому абстракции, создаваемые в рамкахз информатики, должны быть достаточно понятными для того, чтобы быть максимально полезными.  

В контекте программ, которые состоят из большого количества компонентов, для управления их сложностью используются некоторые приемы.
1. Абстракция "черный ящик".
2. Общепринятые интерфейсы как абстракции.
3. Язык программирования (мета-лингвистика.)
#### Черный ящик
"Черный ящик" -- это пример абстракции, с которой многие знакомятся рано благодаря тому, что процедуры и функции в программах часто ассоциируют с этой идеей. Ядро идеи заключается в сокрытии деталей реализации того или иного компонента, что делается с целью упростить комбинирование различных компонентов для получения новых, более сложных компонентов. Другой аспект заключается в том, что у разработчика всегда есть необходимость подчеркнуть связь между своим компонентом и другим более общим компонентом, т.е. язык программирования должен уметь выражать такую обобщенность.

Можно выделить несколько концепций, которые используются для создания "черных ящиков". 
1. Примитивные объекты (процедуры и данные) нужны для выражения значений и управления процессами в конкретном языке программирования.
2. Способы комбинировать объекты (функциональная композиция, алгебраические типы).
3. Способы создания абстракций (определения процедур, абстрактные типы).
4. Способы выражения общепринятых шаблонов и решений (процедуры как значения для других процедур и т.п.)

Для этой главы важной абстракцией является понятие абстрактного типа данных. В качестве примера можно рассмотреть устройство массивов в языке С. Массив представляет собой имя, ссылающееся на адрес того байта, с которого начинается секция, выделенная под весь массив. В этом смысле массивы являются образом виртуальной памяти процесса, так как они полностью повторяют ее структуру: получить доступ к любому из байтов можно по известному адресу, который играет роль индекса. Используя арифметику указателей можно перейти на любой другой адрес – инкрементировать или декрементировать указатель.

Отсюда следует, что массив не является типом данных в привычном понимании. Массив не имеет свойств, присущих только ему, и операций, определенных только для него. Вместо этого массив подвержен операциям, определенным для указателей, и обладает свойствами указателей.
...
Цели обобщенного программирования изначально заключались в декомпозиции программ и гибкой комбинации получившихся модулей. Это было возможно и до введения шаблонов, но в большинстве языков программирования достигалось засчет написания библиотек. Библиотеки, как правило, являются специализированными модулями с конкретным интерфейсом, расчитанным на конкретный род задач. Следовательно, интерфейсы библиотек часто требуют совершенно конкретные типы данных или сами предоставляют новые типы данных для корректной работы алгоритмов из них.
...
Таким образом, если структура содержит несущественные для сравнения части, их нужно игнорировать, а значит побитовое сравнение нужно заменить.
...
Пример выше показывает простой пример определения и использования концепции. Два стандартных предиката integral и floating_point проверяют переданный тип (в данном случае -- шаблонный символ T) на соответствие. Если один из них вернет true, тип, скрывающийся за символом T, можно считать численным. Это новое правило именнуется number с помощью ключевого слова concept. Функция concept_function использует арифметическую операцию, которая определена для всех численных типов, соответственно ее можно обобщить с помощью шаблона, объявив шаблонный аргумент Number как ранее созданную концепцию. 
...
Если вернуться к сравнению динамического и статического полиморфизма в С++, концепции добавляют нюансы, которые нужно иметь ввиду при выборе одного или другого подхода. Ниже показан пример интерфейса, определенного с помощью абстрактного базового класса.
...
Концепции одновременно усиливают контроль за реализацией интерфейсов, определяя конкретные проверяемые ограничения, но ослабевают требования к реализациям по сравнению с абстрактными классами. В случае с абстрактным классом реализация обязана соответствовать ему полностью (не должно быть расхождений в типах данных параметров, минимальном количестве методов и т.д.) независимо от кода пользователя (он может не использовать весь интерфейс, но все равно будет тянуть реализацию даже тех методов, которые в коде не применяются).
    ...
    template <constrained C> void client(C arg);
