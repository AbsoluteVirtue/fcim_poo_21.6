...
Прежде чем окончательно ответить на вопрос, “что такое объектно-ориентированное программирование”, необходимо определиться с тем, какие проблемы оно было призвано решить. Как и со многими другими парадигмами программирования, одно из главных предназначений объектно-ориентированного программирования заключается в переходе от “императивного” к “декларативному” методу организации кода. Несмотря на то, что регулярно появляются новые языки программирования, основные элементы этих языков почти ничем не отличаются от уже существующих. Это, по мнению Джона Бакуса, влечет за собой окостенение мышления, направленного на решение задач -- “мышлением по одному слову за раз”, о чем было сказано в предыдущей главе.
...
1. Очень похожая система использовалась в Sketchpad для описания "мастера" экземпляров.
...
Важность сокрытия деталей реализации для ООП станет понятна позднее. Важность отсутствия операции присваивания уже упоминалась в статье Джона Бакуса. Алан Кей видел в присваивании и другие недостатки. По его собственным словам, "в 60-х много усилий было вложено в развитие идеи абстрактных типов данных, но Smalltalk -- это попытка заменить связывание значений и присваивание на *цели*, чтобы остановить программистов при попытках изменить внутреннее состояние процесса."
...
Другое заимствование из Sketchpad -- разрешение проблемы ограничений -- позволило создать "полиморфные" (обобщенные) процедуры: объекты следили за своим состоянием при инстанциировании, между вызовами процедур, а процедуры служили "целями" для объектов.

> Взятая из архитектуры операционнных систем, идея того, что внутренние структуры данных не разрешается изменять, потому что не у всех пользователей должны быть бесконечные привилегии доступа (и строгой системы типов не достаточно для защиты) -- привела к тому, что все объекты необходимо было сделать "первоклассными гражданами" и защитить всё. ООП -- это стратегия позднего связывания для многих сущностей, и все они позволяют избежать хрупкости и "взрыва" размеров.
...
Вычисление в Smalltalk-71 производилось методом "pattern-matching" аналогично Lisp. Основной цикл позволял выходить из него в произвольных местах (аналогично инструкции goto), создавая структуру, состоящую из событий (event-driven) с возможностью выбора блока с кодом, в котором вычисляется результат в ответ на то или иное событие (аналогично конструкции switch-case).
    to T 'and' :y do 'y' 
    to F 'and' :y do F
...
В первую очередь, для Алана Кея важна изоляция -- инкапсуляция и сокрытие внутреннего состояния процесса. В качестве примера он приводил веб-браузеры, "если ваш браузер закрывался бы при каждой остановке сетевого сервера, к которому он обращался, Internet Explorer никогда бы не достиг версии 2.0". С момента основания ARPANet до возникновения World Wide Web Интернет вырос примерно в 100 миллионов раз, при этом его не приходилось выключать каждый раз, когда происходило расширение сети.  

Четыре техники -- защищенное состояние, полиморфизм, экземпляры классов и методы-как-цели -- в совокупности сделали Smalltalk очень мощным инструментом. Ни одна из этих техник не подразумевает чистый ООП язык. Это можно было уже тогда реализовать и в Lisp, и в Algol 68 (с некоторыми оговорками). Задача объектно-ориентированных языков заключается, скорее, в том, чтобы помогать программисту фокусировать свое внимание на тех идеях и техниках, которые сделают его более продуктивным. 
...
> "Сообщения улучшают изоляцию, потому что принимающая сторона вольна игнорировать сообщения, которые ей не понятны (язык Erlang -- это пример того, как позднее связывание значений и типов данных помогало достичь изоляции). Объекты -- это просто эксперты. Вы говорите им что вам нужно, и они выполняют просьбу как считают нужным. Забудьте про возню с *шаблонами* или *данными с поведением*. Это уже детали реализации.