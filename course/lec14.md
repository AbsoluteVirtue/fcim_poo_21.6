...
~~items = items | std::ranges::filter( [](auto e){ return e.state != hidden; }) std::views::to<std::list>();~~
```C++
...
```
Наконец, последний неожиданный вывод, который можно сделать, заключается в ненадобности структуры *Y*, так как в ней кроме уникального идентификаторы больше нет полезных данных, а значит хранение структуры Base в определенном порядке позволит закодировать поле *id*, например, с помощью хэш-таблицы. Здесь к месту будет цитата Александра Степанова,
...
Кто-то может возразить, "какая мне разница: как долго выполняется та или иная часть программы, 3 секунды или 30 секунд?" Многие известные и не очень специалисты отвечали на этот вопрос, достаточно будет привести слова одного из них, Джона Кармака, которые он сказал в ответ на похожий вопрос,
> it is the more personal pain of seeing a 5% GPU utilization number in production. I am offended by it. As a systems optimization person, I care deeply about efficiency. When you work hard at optimization for most of your life, seeing something that is grossly inefficient hurts your soul. [...] Despite all the complaints I have about our software, millions of people are still getting value out of it. We have a good product. It is successful, and **successful products make the world a better place**.

***
Подвести итог всему курсу хочется другой известной цитатой, принадлежащей Ричарду Гэбриэлу,
> We have lived through three ages of computing—the first was machine coding; the second was symbolic assemblers, interpreter routines, and early compilers; and the third was imperative, procedural, and functional programming, and compiler-based languages. Now we are in the fourth: object-oriented programming. These first four ages featured single-machine applications. Even though such systems will remain important, increasingly our systems will be made up of dozens, hundreds, thousands, or millions of disparate components, partial applications, services, sensors, and actuators on a variety of hardware, written by a variegated set of developers, and it won’t be incorrect to say that no one knows how it all works.

Задача программиста (следовательно -- цель компьютерной программы) заключается в разработке процесса, достигающего конкретные цели. Программисты добиваются этого созданием лексических конструкций, конструируемых из процедур и выражений. Эти конструкции управляют процессом достижения поставленных перед программистом целей. 

Для эффективного выполнения своих задач программист должен хорошо понимать отношения между лексическими конструкциями, которые он пишет (синтаксисом) и поведением процессов, которыми он пытается управлять (семантикой). Для того, чтобы понимать эти отношения, нужно понять как определенные закономерности в процедурах и выражениях порождают закономерности исполнения (поведения) в процессах. Для того, чтобы понимать закономерности в процессах исполнения, нужна зависимость между определением процедуры и поведением процесса. Для этого нужна формальная модель. Как ведет себя тот или иной компьютер на самом деле не имеет большого значения. Модель описывает ожидаемый результат, а не конкретные детали реализации. Всегда можно считать какие-то операции на компьютере примитивными, работая на более высоком - абстрактном - уровне. Как говорил Джеральд Сассман,
> At any level of detail if you look inside this machine you're gonna find that there's multiple levels below that, that you didn't know about. But one of the things we have to learn to do is **ignore details**. The **key to understanding complicated things** is to know what not to look at, and what not to compute, and what not to think.

ООП должно было стать одной такой моделью. Была ли эта попытка успешной судить трудно, даже несмотря на то, что прошло уже больше 50 лет с момента первого упоминания ООП как техники моделирования. (Developing formal semantics for stateful objects is very laborious compared to stateless functions. The original marketing behind OO was to split state in tiny "encapsulated" bits of state spread out all over the place. But this process produces horrible invariants that can't be tackled by the feeble human mind. That was the incredible stupid idea. Fast forward to modern OO where unit-testing (aka poor man's specifications!) best practices encourage stateless methods. Modern OO has become a thin veil for partially applied higher order stateless functions.) В ходе данного курса были озвучены разные мнения на этот счет. Целью данного курса не было убедить читателя в правильности того или иного мнения относительно успешности ООП как парадигмы. Целью было предоставить читателю все детали, чтобы он мог составить свое собственное информированное мнение. 