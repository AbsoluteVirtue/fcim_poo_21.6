...
Из этой цитаты видно, что полиморфизм, в первую очередь, используется для описания поведения функций, и изначально закрепился как термин именно в функциональном программировании. Тем не менее, его всегда упоминают как важнейший аспект и ООП. Идея полиморфизма тесно связана с типами данных. Тип данных определяет представление данных и поведение программы, связанное с данными. Например, целые числа в произвольном языке программирования, как правило, представлены: диапазоном значений, которые данный процессор позволяет отображать или передавать (от -32768 до 32767 для 16-битного числа и т.п.); набором операций, которые можно производить над значениями данного типа (сложение, вычитание, умножение, деление и т.п.); особыми свойствами, присущими данному типу (коммутативность, ассоциативность и т.п.).

Полиморфизм является свойством объектов изменять свое поведение и представление в зависимости от контекста (независимо от типа данных). Вот пример, который Алан Кей приводит в связи с этим.
...
|3|+ 4|7|
|"kitty"|+ "kat"|"kittykat"|
|3 4 5<br>6 7 8| + 4|7&nbsp;&nbsp;&nbsp;&nbsp;8&nbsp;&nbsp;&nbsp;9<br>10 11 12|
...
Как видно из цитаты Карделли выше, со времен языков B и CPL, которые создавались при участии Стрейчи, полиморфизм как концепция оброс своими особенностями. Упомянутые выше виды полиморфизма стоит рассмотреть подробней.
...
Для такого сценария становится возможным следующее выражение:
```C++
void fn(B arg) {}
A a;
fn(a);
B b(a); // вызывается конструктор
fn(b);  
b = a;  // вызывается оператор присваивания
a = b;  // вызывается оператор приведения типа
```
...
что для строк не выполняется, так как они не являются нужной для этого категорией значений:
    ...
    "3" + "4" ~= "4" + "3"
         "34" ~= "43"
В этом примере специальный полиморфизм нарушает фундаментальные математические свойства операции сложения, которая должна быть коммутативной. В большинстве случаев этого следует избегать, так как у пользователей наверняка уже есть выработанная интуиция о природе простых операций. Здесь уместно процитировать А. А. Степанова, “несмотря на то, что типы объектов можно определять любым способом, существуют естественные законы, которые управляют поведением большинства типов; эти законы определяют значение фундаментальных операций на объектах: конструирование, уничтожение, присваивание, обмен, равенство и линейная упорядоченность.”

В современной версии языка перегрузка операторов сравнения может выглядеть так:
```C++
...
Здесь функция cmp представляет собой произвольную функцию, производящую трехстороннее сравнение (например, strcmp из библиотеки string.h). Оператор < перегружался сообразно множеству, которому он принадлежит. Стандартный алгоритм сортировки требует, чтобы сравниваемые типы данных, хранящиеся в аргументе, реализовывали оператор <, который поддерживает строгую частичную упорядоченность (примером такого отношения будет результат лексикографического сравнения двух произвольных последовательностей). Если разработчик проигнорирует данное требование, он потеряет доступ к стандартным алгоритмам, разработанным для языка С++. Если разработчик перегрузит нужный оператор для своего типа данных, но не сохранит математические свойства оператора, результат работы стандартного алгоритма будет непредсказуем.

Другими примерами похожей проблемы могут служить: сравнение чисел с плавающей запятой (частично упорядоченное множество, ЧУМ), сравнение точек в декартовой системе координат (нестрогое ЧУМ) и т.п. Можно себе представить много ситуаций, в которых неправильно реализованное сравнение приводило бы к нежелательным последствиям. Пагинация больших объемов данных, порядок хранения в базах данных, порядок поступления данных из очередей и т.д. – все эти процессы были бы нарушены на фундаментальном уровне.
### Параметризация
...
```
Здесь наглядно видно, как для итерации по спискам используется обычная рекурсия. Если один из списков пустой, то прибавлять к общей сумме нечего, и возвращается 0, в противном случае в списках есть как минимум по одному элементу, которые находятся в “голове” каждого списка, они суммируются с результатом рекурсивного вызова, который принимает остатки обоих списков (“хвосты”) в качестве аргументов. (Нужно обратить внимание на то, что нигде не упоминаются типы данных аргументов и возвращаемого значения. Это делать не нужно, так как компилятор реализован таким образом, чтобы исключить неправильные комбинации типов данных для переданных значений. Он гарантирует, что если типы данных определить в принципе возможно для конкретного случая, будет найдена самая обобщенная форма данной функции. Это позволяет языку использовать статическую систему типов без необходимости указывать типы для всех сущностей в программах.)
### Выявление типов
Выявление типов – это процесс удовлетворения ограничений. В примере выше функция возвращает либо 0, либо результат суммы нуля и произведения значений из обоих списков, т.о. этот результат гарантированно будет из множества, к которому принадлежит 0 – целым числом. Операторы hd, tl и сравнение с [] гарантируют, что параметры a и b будут списками, состоящими из целых чисел. Для создания обобщенных алгоритмов, работающих для любых типов данных, применяются ограничения на типах данных, учитывая определенные фундаментальные требования к значениям, которые будут участвовать в алгоритме.

Их предназначение – четко указывать на семантические особенности алгоритмов непосредственно в коде. (Примером самой универсальной полиморфной функции может быть тождественная функция -- она просто возвращает свой аргумент, соответственно всегда верна для бесконечно большого количества типов данных.) В случае функции prod эти ограничения, во-первых, определяются отношением типов данных к значениям в подвыражениях, и во-вторых, разрешаются последовательным исключением версий кода, которые не могут дать правильного решения задачи. Здесь возможна одна из трех ситуаций.
1. Решений не существует. Это равносильно ошибке компиляции, связанной с неправильным типом данных аргумента.
2. Решений множество. В этом случае возможна ситуация, когда решения неоднозначны (перегрузка), либо когда есть одно “лучшее” решение (полиморфизм).
3. Есть единственное верное решение. Это равносильно заданию всех типов данных явно.
