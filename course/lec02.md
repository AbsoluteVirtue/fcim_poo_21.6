...
Начать стоит с изучения двух базовых элементов, о которых говорит Буч в своей книге: абстрактных данных и классов. И не только потому, что эти понятия сейчас тесно связаны с ООП. Алан Кей активно пользовался этими механизмами во время работы над дизайном языка Smalltalk.
## Базовые элементы объектно-ориентированного дизайна
Когда Алан Кей пришел в подразделение PARC корпорации Xerox, одной из его первых задач было ознакомиться с двумя на тот момент новыми примерами инновационного использования компьютерных технологий. Одной технологией была система компьютерного дизайна Sketchpad Айвана Сазерленда, которая стала одной из самых значимых работ в сфере компьютерных технологий. Другой технологией был язык программирования Simula, использующий новую концепцию "классов как процессов" в качестве основного инструмента организации кода.

Зная о том, что в группе по разработке языка Algol высказывались идеи об использовании "абстрактных типов данных" (которые в более ранних работах известного ученого Дагласа Росса фигурируют под названием "plex"), Кей попытался объединить идею "плексов", классов из Simula и обощенных структур данных из Sketchpad. Конечной целью этой работы было создание "нового типа систем, которые интуитивно понятны даже непрофессионалам". 

Идея архитектуры такой системы заключалась в создании сети компьютерных процессов, которая не позволяет процессам напрямую изменять состояния друг-друга. Вместо этого процессы должны были отправлять друг другу запросы в виде правильно сформированных сообщений. Так, для изменения какого-то значения один процесс посылал сообщение с новым значением другому процессу, который отвечал за данную область памяти. Процесс-получатель читал сообщение и принимал окончательное решение о том, нужно ли изменять значение в памяти или нет. Это позволяло программисту, работающему в такой системе, вместо простых инструкций с присваиванием значений отдельным переменным писать более абстрактные инструкции, описывающие желаемый окончательный результат. Процессы же сами должны были выбирать правильные инструкции для достижения результата. Алан Кей позже говорил,
...
### Объекты
Эта концепция появилась не на пустом месте. Кей был биологом по первому образованию и часто обращался к билогии за вдохновением. Для своей модели он выбрал поведение биологических клеток, которые защищены от внешнего мира мембраной, но могут обмениваться информацией с другими клетками с помощью специальных белков и соответствующих рецепторов. Эта модель была хороша тем, что систему, построенную по такому принципу, можно расширять до бесконечности, получая результат любой сложности (по аналогии с тем, как из одних и тех же клеток с помощью ДНК получаются разные организмы).
...
В такой модели объекты становились "функциями, значениями которых являются поведения".
### Sketchpad – a man-machine graphical communication system
Первоначальная идея дизайна Sketchpad заключалась в том, что Сазерленд задался целью сделать компьютеры "более понятными" для неспециалистов. В те времена .когда компьютеры занимали целое помещение, люди, которые не были компьютерными инженерами, не могли ими пользоваться без посторонней помощи. Sketchpad решал эту проблему для отдельной группы людей -- для инженеров, создавая графический интерфейс (один их первых компьютерных визуальных дизайнов) для создания чертежей на экране с помощью светового пера. Таким образом убивались два зайца: появлялась система, взаимодействие с которой было интуитивно понятно всем, кто с ней соприкасался; при этом чертежники получали инструмент для создания чертежей, который позволял легко исправлять ошибки и бесконечно размножать чертежи.

Чтобы в полной мере оценить важность Sketchpad для ООП, надо отметить, что Алан Кей называл эту работу "наверное, самой значимой диссертационной работой в истории". Стоит только добавить, что Кей также называл Sketchpad первой "объектно-ориентированной" системой. Несколько свойств Sketchpad были напрямую переняты в Smalltalk, для того, чтобы он мог быть по-настоящему объектно-ориентированным языком.
#### Master - instance
Чертежи, создаваемые пользователем, организовывались в иерархию, в которой был главный чертеж (master drawing), а изменения этого шаблона отражались на всех его копиях. Копии или "экземпляры" (instances) создавались на основе главного чертежа, повторяя все его свойства, причем изменения одной копии никак не отражались на остальных элементах чертежа. Экземпляры можно было использовать для создания новых "рекурсивных" иерархий любой степени вложенности. Этот принцип позже лег в основу иерархий наследования, которые создавались в Smalltalk.
#### Constraints
...
#### n-component
Наверное, самой интересной для Алана Кея частью дизайна Sketchpad была организация данных в структуры в памяти компьютера. Так как чертежи состоят из двух видов элементов: линий и пересечений линий -- все данные в программе можно было хранить в виде одинаковых структур, которые отличались незначительно (координатами для отображения на экране). Для хранения данных в программе используется блок из нескольких компонентов, а весь чертеж представлялся списком из нескольких блоков. Так, иерархическая структура родительского чертежа и дочерних экземпляров хранилась в памяти в виде плоской структуры (https://www.youtube.com/watch?v=ShSGHb65f3M), с которой намного легче работать. При этом, пользователи не взаимодействуют с фундаментальной структурой, в процессе черчения пользуясь только пером и кнопками на компьютере.

В итоге, система комбинировала операции для взаимодействия с каждым отдельным элементом чертежа, сам чертеж ссылался на нужную операцию (это было заимствовано из "плексов" Росса). Например, операция "отображения" получала инструкцию от списка элементов, которые нужно было отобразить, и вызывала соответствующую подпрограмму для соответствующего элемента. Т.е., система имела три уровня: Уровень взаимодействия пользователей с системой, уровень хранения данных и подпрограмм в памяти, и уровень абстракции между другими двумя уровнями, который организовывал взаимодействие пользователя с данными и операциями. 

Пользой такого подхода -- разделения на "детальное" и "обобщенное" -- была легкость в расширении деталей реализации без затрагивания особенностей взаимодействия пользователей с программой. 
...
Это слова одного из разработчиков языка SIMULA, который разрабатывался в Норвегии начиная с 1962 года. Первоначально предназначенный для симуляции событий в сложных индустриальных системах (конкретней, в "сетях дискретных событий"), язык со временем превратился в систему, позволявшую создавать иерархии сущностей в рамках какой-то классификации. 

Язык был надстройкой для языка Algol 60, что в некоторой мере ограничивало возможности разработчиков компилятора для SIMULA (например, работа с памятью в Algol вынуждала выбирать не самые лучшие решения для реализации компилятора). Как говорил другой разработчик в том же 1962 году:
...
Так как процессы обращались друг к другу напрямую, необходимо было переделать механизмы доступа к атрибутам процесса. Была заимствована идея "классов-записей" Хора, которую разрабатывали для Algol. Компилятор проверял доступность характеристик процесса через наличие правильной ссылки, Во время работы программы к ссылкам на процесс имели доступ его подпроцессы ("подклассы").
...
Алан Кей с большим энтузиазмом перенял идею "классов-процессов" в свой дизайн. Возможность описывать поведение объектов в соответствующих классах хорошо накладывалась на модель системы, где объекты являются независимыми сущностями. Для сравнения ниже приводится похожая конфигурация классов в языке Smalltalk.
```Smalltalk
class Page;
begin
...
Тем не менее, у Sketchpad было одно свойство, которое Алан Кей не стал переносить в свою систему. Как он позже объяснял:
...
> “из Simula я выделил то, что переменные и присваивание можно было заменять на цели. Последнее, чем программист должен заниматься – это манипуляция внутренним состоянием, даже если оно представлено метафорически. Вместо этого объекты должны быть представлены как сущности с высокоуровневым поведением, которые лучше подходят для использования в качестве динамических компонентов”.
...
Объект - это описание вычислительного процесса или семейства процессов, которое скрывает свои данные от внешней среды и может работать независимо от других объектов, взаимодействуя с ними с помощью сообщений, отправленных согласно какому-то протоколу. В первой части данного курса рассматриваются три главных элемента, которые использовал в своей работе Алан Кей: абстрактные типы данных, классы и обобщенные структуры данных.

Одним из главных направлений в дизайне языка Smalltalk был отказ от использования абстрактных типов данных в их привычном понимании (как продолжения идеи "плексов" Росса). Дизайн требовал абсолютной защиты состояния объекта от внешнего воздействия (например, полный отказ от “сеттеров” и “геттеров”). Результатом такого подхода является стратегия позднего связывания (тип объекта определяется в момент обращения к нему на основании формы сообщения, что в других языках достигается засчет динамического полиморфизма. Алану Кею хотелось добиться обобщенного поведения объектов, которое зависит от символов в сообщении, а не от типа данных самого объекта. 
...
В итоге, Алан Кей остановился на таком дизайне (очевидным источником вдохновения был Lisp, менее очевидным -- "актеры" Карла Хьюэтта) для своего языка программирования, в котором:
1. Все являлось объектом.
2. Объекты взаимодействуют друг с другом посредством сообщений.
3. Каждый объект обладает своим участком памяти (который тоже является объектом).
4. Каждый объект -- это экземпляр класса (который тоже сам является объектом).
5. Класс хранит описание поведения всех своих экземпляров (в виде списка объектов).
6. Чтобы вычислить результат программы, необходимо активировать первый объект в списке объектов программы, а остальные объекты становятся частью сообщения для первого объекта.
